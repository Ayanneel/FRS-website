...
public class DaoTicket 
{
	public int insertTicketDetails(TicketBean t) throws Exception
	{
		try 
		{
			...
			pstmnt.setString(13, t.getAgeList());
	...
	public ArrayList<PassengerDetailsBean> returnAllPassengers() throws Exception
	{
		try 
		{
			ArrayList<PassengerDetailsBean> list= new ArrayList<PassengerDetailsBean>();
			DBUtil db = new DBUtil();
			Connection conn = db.connectDatabase();
			String sql="select pass_names,age,flight_id,t_date from ticket_info";
			PreparedStatement ptsmnt = conn.prepareStatement(sql);
			ResultSet rs = ptsmnt.executeQuery();
			while(rs.next())
			{
				
				String[] passNames=rs.getString(1).split(", ");
				String [] ageList = rs.getString(2).split(", ");
				for( int i=0;i<rs.getString(1).split(", ").length;i++)
				{
					PassengerDetailsBean pdb = new PassengerDetailsBean();
					pdb.setName(passNames[i]);
					pdb.setAge(ageList[i]);
					pdb.setFlightId(rs.getInt(3));
					pdb.setBookingDate(rs.getDate(4).toString());
					list.add(pdb);	
		...
		{
			System.out.println("mistake @ returnAllPassengers");
			return null;
		}
