...
public class DaoFlight 
{
	...
	public void deleteRow(int flightId,String tableName) throws Exception
	{
		DBUtil db = new DBUtil();
		Connection conn = db.connectDatabase();
		String sql="delete from $tableName where flight_id=?";
		String sqlQuery = sql.replace("$tableName", tableName);
		PreparedStatement pstmnt = conn.prepareStatement(sqlQuery);
		pstmnt.setInt(1, flightId);
		pstmnt.executeUpdate();
	}
	
	public int updateFlight(FlightBean f , String table) throws Exception
	{
		try
		{
			DBUtil db= new DBUtil();
			Connection conn = db.connectDatabase();
			String sql = "update $tableName set from_location=?, to_location=?, duration=?, total_seats=?, distance=?, price=? where flight_id=?"; 
			String sqlQuery = sql.replace("$tableName", table);
			PreparedStatement pstmnt = conn.prepareStatement(sqlQuery);
			pstmnt.setString(1, f.getSource());
			pstmnt.setString(2, f.getDestination());
			pstmnt.setFloat(3,f.getDuration());
			pstmnt.setInt(4, f.getTotalSeats());
			pstmnt.setFloat(5, f.getDistance());
			pstmnt.setInt(6, f.getPrice());
			pstmnt.setInt(7, f.getFlightId());
			return pstmnt.executeUpdate();
