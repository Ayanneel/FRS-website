...
public class BookingServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		for(String j: naam)
		{
			age+=request.getParameter("age"+j)+", ";
		}
		System.out.println(age);
		String finalPassengerNames= passengers.substring(0, passengers.length()-2);
		String finalAge = age.substring(0,age.length()-2);
		...
		try {
				...
				{
					if(vc.generateTicket(flightId, Integer.parseInt(passId), depTime,arrTime, date,finalPassengerNames,bookingInt,source,destination,email,finalAge)==1)
						{
